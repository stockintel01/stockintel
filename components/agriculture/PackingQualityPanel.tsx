...
export function PackingQualityPanel({ records, events, config, stations, userId, userName, canManage, initialSection = 'queue', shippedBoxesByRecord, onRecord, onSaveConfig }: Props) {
  ...
  return <div className="space-y-4">
    ...
    {selected && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/55 p-3" role="dialog" aria-modal="true" aria-label={mode === 'correction' ? 'Correct packing record' : 'Quality inspection'}>
      <Card className="max-h-[92vh] w-full max-w-3xl overflow-y-auto"><CardHeader><CardTitle className="flex items-center justify-between">{mode === 'correction' ? 'Correct packing and quality totals' : mode === 'rework_resolution' ? 'Resolve rework' : 'Inspect packed boxes'}<button aria-label="Close inspection" onClick={() => setSelected(null)}><X className="h-5 w-5" /></button></CardTitle></CardHeader><CardContent className="space-y-4">
        <div className="rounded-lg border bg-muted/40 p-3 text-sm">{selected.stationName} · {selected.produce} · {selected.packedBoxes} packed boxes{(shippedBoxesByRecord.get(selected.id) ?? 0) > 0 ? ` · ${shippedBoxesByRecord.get(selected.id)} already shipped` : ''}</div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {([['packageType', 'Package type', packageType, setPackageType, standards.packageTypes], ['packageSize', 'Package size', packageSize, setPackageSize, standards.packageSizes], ['grade', 'Quality grade', grade, setGrade, standards.qualityGrades]] as const).map(([, label, value, setter, options]) => <div key={label}><Label>{label}</Label><select disabled={(selected.inspectedBoxes ?? 0) > 0} className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm disabled:opacity-70" value={value} onChange={event => setter(event.target.value)}>{options.map(option => <option key={option}>{option}</option>)}</select></div>)}
